...
class V12Config:
    ...
    edge_weight_strength: float = 1.25
    edge_weight_power: float = 2.0
    focal_prior_mode: str = "prior"  # free, prior, or fixed
    lambda_focal_prior: float = 0.15
    lambda_center_prior: float = 0.05
    shuffle_every_epochs: int = 1
...
class MaskingV12Analyzer:
    ...
    def fit(
        ...
        reuse_model = bool(warm_start and self.model is not None)
        if reuse_model:
            model = self.model.to(device)
        else:
            model = AsymmetricVignettingPINNV12(hidden_dim=cfg.hidden_dim, init_f_norm=init_f_norm).to(device)
        focal_prior_mode = str(cfg.focal_prior_mode).lower().strip()
        if focal_prior_mode not in {"free", "prior", "fixed"}:
            raise ValueError(f"Unsupported focal_prior_mode: {cfg.focal_prior_mode}")
        if focal_prior_mode == "fixed":
            model.f_raw.requires_grad_(False)
        else:
            model.f_raw.requires_grad_(True)
        ...
        phys_params = [model.i0_raw, model.cx_raw, model.cy_raw, model.ax_raw, model.ay_raw, model.phi_raw]
        if focal_prior_mode != "fixed":
            phys_params.insert(0, model.f_raw)
        ...
        edge_r = torch.sqrt(torch.clamp(x_all ** 2 + y_all ** 2, min=0.0))
        edge_w_all = 1.0 + float(cfg.edge_weight_strength) * torch.clamp(edge_r, 0.0, 1.0) ** float(cfg.edge_weight_power)
        edge_w_all = edge_w_all / torch.clamp(torch.mean(edge_w_all), min=1e-6)
        focal_prior = torch.tensor(float(init_f_norm), dtype=torch.float32, device=device)
        center_prior = torch.tensor([float(cx_norm), float(cy_norm)], dtype=torch.float32, device=device)
        ...
        print("\n=== maskingV1.4 Training ===")
        print(
            f"[-] Device={device} | Samples={n} | Epochs={cfg.epochs} | Batch={cfg.batch_size} | "
            f"Hidden={cfg.hidden_dim}"
        )
        print(
            "[-] Upgrades: edge-weighted data/anchor loss + focal prior/fixed mode + "
            "center prior + V1.3 decomposition."
        )
        if reuse_model:
            print("[-] Warm start: continuing from existing V1.4 model state.")

        log_every = max(1, int(log_every))
        shuffle_every = max(1, int(getattr(cfg, "shuffle_every_epochs", 1)))
        perm = None

        for epoch in range(1, cfg.epochs + 1):
            if perm is None or (epoch - 1) % shuffle_every == 0:
                perm = torch.randperm(n, device=device)
            s_l = s_ld = s_lp = s_ll = s_lr = s_la = s_lgb = s_lw = s_lf = s_lc = 0.0
            nb = 0
            ...
            for sidx in range(0, n, cfg.batch_size):
                ...
                loss_data = torch.mean(edge_w * torch.mean((i_pred - tgt) ** 2, dim=1, keepdim=True))
                ...
                loss_anchor = torch.mean(edge_w * (v_pred - v_anchor) ** 2)
                ...
                f_norm_now, _, cx_now, cy_now, _, _, _ = model.physics_params()
                if focal_prior_mode == "prior":
                    loss_focal = torch.mean((f_norm_now / torch.clamp(focal_prior, min=1e-6) - 1.0) ** 2)
                else:
                    loss_focal = torch.zeros((), dtype=torch.float32, device=device)
                loss_center = (cx_now - center_prior[0]) ** 2 + (cy_now - center_prior[1]) ** 2
                ...
                loss = (
                    ...
                    + cfg.lambda_focal_prior * loss_focal
                    + cfg.lambda_center_prior * loss_center
                )
                ...
                s_lf += float(loss_focal.item())
                s_lc += float(loss_center.item())
                nb += 1
            ...
            avg_lf = s_lf / max(nb, 1)
            avg_lc = s_lc / max(nb, 1)
            ...
            if epoch == 1 or epoch % log_every == 0 or epoch == cfg.epochs:
                ...
                print(
                    ...
                    f"Lgb={avg_lgb:.6e} Lw={avg_lw:.6e} Lf={avg_lf:.6e} Lc={avg_lc:.6e} | "
                ...
                if progress_json:
                    progress = {
                        ...
                        "priors": {
                            "focal_mode": focal_prior_mode,
                            "focal_mm": float(focal_prior.item() * self.sensor_half_diag_mm),
                            "center_norm": [float(center_prior[0].item()), float(center_prior[1].item())],
    ...
    def _predict_rotated_lighting(self, k: int, chunk: int = 300000):
        x_map, y_map = self._full_xy()
        if k == 0:
            rx, ry = x_map, y_map
        elif k == 1:
            ...
            raise ValueError(f"Unsupported capture index {k}")

        flat_x = rx.ravel().astype(np.float32, copy=False)
        flat_y = ry.ravel().astype(np.float32, copy=False)
        out = np.empty_like(flat_x, dtype=np.float32)
        ...
        with torch.no_grad():
            for i in range(0, flat_x.shape[0], chunk):
                tx = torch.from_numpy(flat_x[i:i + chunk, None]).to(self.device)
                ty = torch.from_numpy(flat_y[i:i + chunk, None]).to(self.device)
                out[i:i + chunk] = self.model.light_from_xy(tx, ty).squeeze(1).cpu().numpy()

        l = out.reshape(self.h, self.w)
        return np.clip(l, 1e-6, 1e6).astype(np.float32, copy=False)
    ...
    def predict(self):
        ...
        for k in range(4):
            lk = self._predict_rotated_lighting(k)
...
def main():
    parser = argparse.ArgumentParser(description="maskingV1.4: edge-weighted decomposition PINN with focal/center priors")
    ...
    parser.add_argument("--edge-weight-strength", type=float, default=1.25)
    parser.add_argument("--edge-weight-power", type=float, default=2.0)
    parser.add_argument("--focal-prior-mode", type=str, default="prior", choices=("free", "prior", "fixed"))
    ...
    cfg = V12Config(
        ...
        focal_prior_mode=str(args.focal_prior_mode),
