...
def load_module(path: Path):
    spec = importlib.util.spec_from_file_location("masking_v132_cpu_fast", str(path))
...
def save_preview_gray(path: Path, image: np.ndarray) -> None:
    ...
    path.write_bytes(cv2.imencode(".png", np.round(arr * 255.0).astype(np.uint8))[1].tobytes())
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CPU-fast two-stage maskingV1.3.2 test on 85mm f1.8.")
    parser.add_argument("--output-root", type=Path, default=Path("maskingV132_cpu_fast_twostage_85"))
    ...
    module = load_module(Path("maskingV1.3.2.py"))
    ...
    print(
        "[run] V1.3.2 CPU fast two-stage 85mm | "
        f"threads={module.torch.get_num_threads()} batch={args.batch_size} "
        f"shuffle_every={args.shuffle_every_epochs}",
        flush=True,
    )
