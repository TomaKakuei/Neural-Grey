...
def load_module(path: Path):
    spec = importlib.util.spec_from_file_location("masking_v14_free_edge100", str(path))
...
def run_case(module, case: dict, args, output_root: Path) -> dict:
    trial_dir = output_root / case["case_id"]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run V1.4 free focal + edge 1.00 at 300 epochs.")
    parser.add_argument("--output-root", type=Path, default=Path("maskingV14_free_edge100_e300_3cases"))
    parser.add_argument("--epochs", type=int, default=300)
    parser.add_argument("--batch-size", type=int, default=8192)
    parser.add_argument("--sample-count", type=int, default=240000)
    parser.add_argument("--hidden-dim", type=int, default=96)
    parser.add_argument("--edge-weight-strength", type=float, default=1.0)
    ...
    rows = []
    for case in CASES:
        print(f"\n=== {case['case_id']} | V1.4 free focal edge={args.edge_weight_strength} e{args.epochs} ===", flush=True)
