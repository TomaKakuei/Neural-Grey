"""
Traditional Mask V3 (commented copy)

This version upgrades the fitting expression power while staying classical.
Main upgrades:
1) Move from radial-bin-mean core fitting to pointwise fitting.
2) Add switchable radial models:
   - poly basis
   - spline-like basis (SciPy-free).
3) Add lighting mesh mode with smoothness regularization.
4) Keep older polynomial/quadratic paths as compatibility fallback.

Notes:
- Designed to reduce information loss from radial binning.
- Improves non-learning baseline flexibility and stability.
"""
import argparse
