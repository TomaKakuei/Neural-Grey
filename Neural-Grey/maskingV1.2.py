"""
maskingV1.2 (commented copy)

Purpose:
- Physics-informed asymmetric vignetting fitting pipeline with rotated captures.

Core modeling:
1) V(x, y): vignetting/shading field.
2) L(x, y): illumination field.
3) Per-frame nuisance parameters (gain/bias and rotation-aware mapping).
4) Joint objective using image reconstruction + geometric/physical constraints.
...
class V12Config:
    ...
    epochs: int = 600
    batch_size: int = 8192
    sample_count: Optional[int] = 120000
    hidden_dim: int = 128
    lr_net: float = 8e-4
    lr_phys: float = 8e-5
    lr_light: float = 6e-5
    lr_gain_bias: float = 6e-5
    lambda_phys: float = 1.2
    lambda_light: float = 0.06
    lambda_residual: float = 0.08
    lambda_anchor: float = 0.12
    lambda_gain_bias: float = 0.15
    lambda_warp: float = 0.35
...
class MaskingV12Analyzer:
    ...
    def load_from_synthetic_dir(self, root_dir: str):
        ...
        h, w = caps[0].shape
        if h != w:
            raise ValueError(f"maskingV1.2 expects square images for exact 90deg coordinate transforms, got {h}x{w}.")
    ...
    def fit(self, focal_length_mm: float, sensor_diag_mm: float, cfg: V12Config, seed: int = 42):
        ...
        for epoch in range(1, cfg.epochs + 1):
            ...
            if epoch == 1 or epoch % 100 == 0 or epoch == cfg.epochs:
                ...
                print(
                    ...
                    f"L={s_l/max(nb,1):.6e} Ld={s_ld/max(nb,1):.6e} Lp={s_lp/max(nb,1):.6e} "
                    f"Ll={s_ll/max(nb,1):.6e} Lr={s_lr/max(nb,1):.6e} La={s_la/max(nb,1):.6e} "
                    f"Lgb={s_lgb/max(nb,1):.6e} Lw={s_lw/max(nb,1):.6e} | "
...
def main():
    ...
    parser = argparse.ArgumentParser(description="maskingV1.2: robust preprocessing + decomposition PINN")
